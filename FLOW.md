...
11. There is currently no way to *create* a portfolio item through the API at all — every item in the catalog today came from the same seed migration that created the demo professional account. Letting a professional upload their own work samples is a planned but not-yet-built feature.
